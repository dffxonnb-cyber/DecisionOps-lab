...
DecisionOps Lab is a product analytics and analytics engineering portfolio project. It turns synthetic product event data into SQL models, quality checks, experiment evidence, guardrail review, decision memos, scenario testing, and reviewer-facing reports.

## One-Line Summary

I built a reproducible workflow that moves from raw product events to a decision-ready experiment recommendation, with data quality checks and D7 revisit guardrails before any final recommendation is made.
...
> Can an analytics workflow turn raw event data into a reproducible, quality-checked, decision-ready experiment memo?
...
However, the project does not use activation alone. It also checks D7 revisit behavior as a guardrail.
...
| Layer | Output |
| --- | --- |
| Data generation | Reproducible synthetic raw CSVs |
...
| Guardrail review | D7 revisit delta and guardrail status |
| Decision memo | Ship / Retest / Hold / Investigate recommendation |
| Reviewer report | Static HTML report for quick portfolio review |
| Scenario matrix | Five synthetic scenarios to test decision behavior |
...
| `reports/experiment_result.json` | Stores experiment and guardrail evidence |
...
> DecisionOps Lab is a reproducible product analytics workflow. I generated synthetic product event data, modeled it through raw, staging, intermediate, and mart layers, checked data quality, analyzed an onboarding experiment, used D7 revisit as a guardrail, and generated decision memos and reviewer reports. I also added scenario testing so the workflow proves Ship, Retest, Hold, and Investigate outcomes instead of always producing one fixed result.
