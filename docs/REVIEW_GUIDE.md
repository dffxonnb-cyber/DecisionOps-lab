...
DecisionOps Lab is not a dashboard-only analytics project. It demonstrates an end-to-end product analytics workflow: raw product events become SQL models, data quality checks, experiment evidence, guardrail review, decision memo, scenario matrix, and reviewer-facing artifacts.
...
4. Read `docs/DECISION_RULES.md` to understand the recommendation logic.
5. Read `docs/MART_LAYER.md` to understand the final SQL mart structure.
...
| `reports/experiment_result.json` | A/B test and guardrail evidence |
| `reports/decision_memo.md` | Final recommendation memo |
...
| Product analytics | Activation, retention, experiment, and guardrail metrics |
| Data quality | PASS / WARN / FAIL quality report before interpretation |
| Experiment analysis | A/B lift, p-value, confidence interval, segment diagnostics |
...
| --- | --- | --- |
| `strong_positive` | Ship | Primary metric improves and D7 revisit remains stable |
...
> I built DecisionOps Lab to show the layer between raw analysis and product decision-making. The workflow checks data quality before interpreting an experiment, uses D7 revisit as a guardrail against short-term metric chasing, and runs multiple synthetic scenarios to prove the recommendation logic is not hard-coded.
